package com.kang.section01.timecomplexity;
...
*
* 시간 복잡도란?
* - 알고리즘 ( 로직 ) 해결하는데 걸리는 시간의 양을 나타내는 척오
* - 입력 크기 (n)에 대한 함수로 표현
...
public class Application {
  public static void main(String[] args) {
    ...
    System.out.println("로그 시간 O(log n) 결과 인덱스 : " + binarySearch(arr, 4));
    System.out.println("");
   ...
   * - 주로 탐색 공간을 절반씩 줄여 나가며 탐색하는 알고리즘에서 나타난다.
  ...
  private static int binarySearch(int[] arr, int target) {
    ...
    while (left <= right) {
      ...
      // 중간 찾기
      // int mid = left + (right - left) / 2; // <- 오버플로우를 방지하기 위한
    ...
    // 찾으려는 값이 배열내에 없거나 또는 정렬 안됨
    return -1;
  }

  /* 지수 시간
   * 지수 시간 0(2^n)
   * - 입력 크기 (n)이 증가할 때 마다 2의 n제곱에 비례하여 시간이 기하급수적으로 증가하는 경우를 의미
   *
   */
